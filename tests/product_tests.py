...
def api_get(path="", params=None, allow_redirects=False):
    try:
...
def assert_auth_protected_response(response):
    assert response.status_code in (301, 302, 303, 307, 308, 401), response.text

    if response.status_code in (301, 302, 303, 307, 308):
        location = response.headers.get("Location", "")
        assert location, "Для редиректа ожидается заголовок Location"
        assert any(
            marker in location
            for marker in ("accounts.google.com", "/oauth2/", "/login/oauth2/", "/auth/")
        ), location
...
def test_get_products_invalid_price_format_returns_current_api_behavior():
    ...
    assert_auth_protected_response(response)
